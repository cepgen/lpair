...
Int_t main() {
  ...
  for (Int_t i=0; i<1e6; i++) {
    ...
    for (Int_t p=0; p<pyjets_.n; p++) {
      ...
      PID[npart] = pyjets_.k[1][p];
      parentid[npart] = pyjets_.k[2][p];
      //cout << p << "\t" << PID[npart] << "\t" << firstdaughterid << "\t" << lastdaughterid << endl;
      isstable[npart] = pyjets_.v[4][p]==0.;
      px[npart] = pyjets_.p[0][p];
      py[npart] = pyjets_.p[1][p];
      pz[npart] = pyjets_.p[2][p];
      E[npart] = pyjets_.p[3][p];
      M[npart] = pyjets_.p[4][p];
      pt[npart] = sqrt(pow(px[npart],2)+pow(py[npart],2));
      
      cout << "--> PDG = " << PID[npart] << ", " << p << " = " << pyjets_.p[0][p] << "\t" << pyjets_.p[1][p] << "\t" << pyjets_.p[2][p] << "\t" << pyjets_.p[3][p] << "\t" << pyjets_.p[4][p] << endl;
